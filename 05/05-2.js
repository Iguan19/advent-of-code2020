...
fs.readFile('input.txt', (err, data) => { 
    ...
    */
    ...
    const missingSeat = (seatsArray) => {
        let seatsIdArray = seatsArray.map(seat =>{
            return seatId(seat)
        })
        seatsIdArray = seatsIdArray.sort(function(a, b){return a-b})

        for(let i=0; i<seatsIdArray.length; i++){
            if(seatsIdArray[i+1] - seatsIdArray[i] != 1){
                return seatsIdArray[i] + 1
            }
        }
    }
    ...
    console.log(missingSeat(seatsArray))
})